...
#define PERIOD            1e6 /* 1000us (in ns) */
#define RELATIVE_DEADLINE 1e6 /* 1000us (in ns) */
#define EXEC_COST         1 /* budget enforcement is disabled */
		...
		case CTXTSWITCH:
			...
			break;
               case WAKEUP:
                       ...
                       break;
               case WAKEUPRT:
                       ret = settracer("wakeup_rt_litmus");
                       break;
		default:
			if (strlen(tracer)) {
...
void *timerthread(void *param)
{
	...
	rtparam.exec_cost = EXEC_COST;
	rtparam.period = par->interval * 1000;
	rtparam.relative_deadline = par->interval * 1000;
	rtparam.budget_policy = NO_ENFORCEMENT;
	rtparam.cls = RT_CLASS_SOFT;
	rtparam.priority = LITMUS_HIGHEST_PRIORITY;

	init_rt_thread();

	if (par->cpu != -1) {
		rtparam.cpu = par->cpu;
		if (be_migrate_to_cpu(par->cpu) < 0) {
			fatal("Could not set CPU affinity to CPU #%d\n", par->cpu);
		}
	}

	if (set_rt_task_param(gettid(), &rtparam) < 0) {
		fatal("timerthread%d: failed to set rt_param. (e=%llu, p=%llu, d=%llu)\n", par->cpu, rtparam.exec_cost, rtparam.period, rtparam.relative_deadline);
	}

	if (task_mode(LITMUS_RT_TASK) != 0) {
		fatal("timerthread%d: failed to change task mode to RT.\n", par->cpu);
	}
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < num_threads; i++) {
		...
		if (!litmus_is_valid_fixed_prio(priority))
			fatal("Invalid priority number: %d.\n", priority);
	...
	while (!shutdown) {
		...
		if (!policystr)
			policystr = "LitmusRT"; //policyname(policy);
